...
## 2) Point-estimators (что оценивает `V̂`)
...
`Bootstrap CI` не задаёт новую формулу `V̂`.
...
2. Считаем `V̂^(b)` тем же оценщиком.
3. CI берём по квантилям `α/2` и `1-α/2`.
...
Это слой статистического вывода поверх выбранного point-estimator (`V̂`).

Если CI считается отдельным шагом поверх DR, это может выглядеть как «ещё один метод», но математически это скорее «pipeline для инференса», а не новый `V_hat`.
