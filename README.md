...
`%(message)s`), поэтому дополнительные настройки не требуются. При
построении бутстрэп‑интервалов внутренние итерации временно отключают
логирование оценщиков, чтобы избежать сотен повторяющихся сообщений в
консоли — итоговые значения и доверительные интервалы остаются в выводе.
