...
Оценщики поддерживают подробное логирование на русском языке, которое можно включать по необходимости.
...
Функции‑оценщики принимают аргумент `log`. По умолчанию он равен `False`,
поэтому расчёт проходит без дополнительных сообщений. Передайте `log=True`,
чтобы увидеть подробный ход вычислений: начало работы оценщика, проверки
корректности пропенсити, значение ESS, долю клиппинга и итоговую метрику.

```python
V_ips, ess_ips, clip_ips = ips_value(df, piB_taken, pA_taken, target="accept", log=True)
```

В Jupyter-туториале логирование включено тем же аргументом в четвёртой ячейке.
