...
`%(message)s`), поэтому дополнительные настройки не требуются. Во время
бутстрэпа информационные и предупреждающие сообщения автоматически
подавляются, чтобы не захламлять вывод сотнями повторяющихся строк.
