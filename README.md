...
Функции‑оценщики выводят диагностические сообщения на уровне `INFO`:
сообщают о начале расчётов, значении ESS и доле клиппинга. По умолчанию
логирование уже настроено (формат `%(message)s`), поэтому дополнительные
